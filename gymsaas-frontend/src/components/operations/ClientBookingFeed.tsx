              ...
              <div key={cls.id} className={`flex items-center p-4 bg-white transition-colors hover:bg-gray-50 ${cls.hasBooked ? 'bg-gray-50' : ''}`}>
                ...
                <div className="ml-4 flex flex-col items-end justify-center min-w-[80px]">
                  {cls.isCancelled ? (
                    ...
                    <button onClick={() => triggerCancel(cls)} className="w-10 h-10 rounded-full border-2 border-black flex items-center justify-center text-black hover:bg-gray-100 transition">
                      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                    </button>
                  ) : (
                    <>
                      <button onClick={() => triggerReserve(cls)} className="text-sm font-black text-black tracking-wider hover:underline">
                        RESERVAR
                      </button>
                      <div className="flex items-center text-xs text-gray-500 font-bold mt-1">
                        <svg className="w-3.5 h-3.5 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" /></svg>
