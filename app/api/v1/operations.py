...
# CORREGIDO: Se remueve prefix="/operations" para que combine limpiamente con el prefix="/api/v1" de main.py
router = APIRouter(tags=["Operaciones y Reservas"])
...
async def list_classes(
    ...
    response = []
    for cls in classes:
        # CORREGIDO: Se usa Booking.session_id (nombre real en schema.py)
...
async def reserve_class(
    ...
    booking_opens_at = datetime.combine(class_session.session_date, class_session.start_time) - timedelta(hours=22)
    booking_opens_at = booking_opens_at.replace(tzinfo=CARACAS_TZ)
    class_start_dt = datetime.combine(class_session.session_date, class_session.start_time).replace(tzinfo=CARACAS_TZ)
    ...
    if class_session.room:
        ...
        if class_session.room not in allowed_rooms:
            raise HTTPException(
                status_code=403, 
                detail=f"Tu membresía actual ({current_plan}) no incluye acceso a la sala {class_session.room}."
            )

    # 5. Comprobar cupos disponibles usando Booking.session_id
    count_result = await db.execute(
        select(func.count(Booking.id))
        .filter(Booking.session_id == payload.class_id, Booking.status == "RESERVED")
    )
    reserved_count = count_result.scalar() or 0

    if reserved_count < class_session.max_capacity:
        booking_status = "RESERVED"
    else:
        booking_status = "WAITLISTED"

    # 6. Descontar crédito atómicamente
    subscription.current_weekly_credits -= 1

    # 7. Registrar reserva usando Booking.session_id
    new_booking = Booking(
        user_id=payload.user_id,
        session_id=payload.class_id,
    ...
    return {
        ...
        "class_id": new_booking.session_id,
...
async def cancel_booking(
    ...
    class_result = await db.execute(select(ClassSession).filter(ClassSession.id == booking.session_id))
    class_session = class_result.scalars().first()
    
    class_start_dt = datetime.combine(class_session.session_date, class_session.start_time).replace(tzinfo=CARACAS_TZ)
    cancellation_deadline = class_start_dt - timedelta(hours=1)
    ...
    if booking.status == "WAITLISTED":
        ...
        if now_caracas <= cancellation_deadline:
            ...
            # PROMOCIÓN DE LISTA DE ESPERA (FIFO) usando session_id
            waitlist_result = await db.execute(
                select(Booking)
                .filter(Booking.session_id == class_session.id, Booking.status == "WAITLISTED")
...
async def get_class_roster(
    ...
    """
    Devuelve la lista de atletas (nombre, apellido y correo) inscritos en una clase específica.
    """
    query = (
        select(User)
        .join(Booking, Booking.user_id == User.id)
        .filter(Booking.session_id == class_id, Booking.status.in_(["RESERVED", "ATTENDED", "WAITLISTED"]))
    )
    result = await db.execute(query)
...
async def close_class_session(
    ...
    result = await db.execute(
        select(Booking)
        .filter(Booking.session_id == class_id)
